...
######################
CONFIG_PATH = "./experiments/linear0.yaml"
######################
...
def predict_df(start_date_str: str, end_date_str: str, path_to_ips_file: str, verbose=False):
    ...
    hist_ips_df['GeoID'] = hist_ips_df['CountryName'] + \
        '__' + hist_ips_df['RegionName'].astype(str)
    # Fill any missing NPIs by assuming they are the same as previous day
    for npi_col in NPI_COLS:
        hist_ips_df.update(hist_ips_df.groupby(
            ['CountryName', 'RegionName'])[npi_col].ffill().fillna(0))

    # Intervention plans to forecast for: those between start_date and end_date
    ips_df = hist_ips_df[(hist_ips_df.Date >= start_date)
                         & (hist_ips_df.Date <= end_date)]
    ...
    hist_cases_df['GeoID'] = hist_cases_df['CountryName'] + \
        '__' + hist_cases_df['RegionName'].astype(str)
    # Add new cases column
    hist_cases_df['NewCases'] = hist_cases_df.groupby(
        'GeoID').ConfirmedCases.diff().fillna(0)
    ...
    for g in ips_df.GeoID.unique():
        ...
        print(hist_cases_gdf.head())
        print('lst known date', last_known_date)
        continue

        # Make prediction for each day
        geo_preds = []
        # Start predicting from start_date, unless there's a gap since last known date
        current_date = min(last_known_date +
                           np.timedelta64(1, 'D'), start_date)
        days_ahead = 0
        while current_date <= end_date:
            ...
            print(current_date, start_date, geo_preds, g)
            ...
            past_npis = np.append(
                past_npis, future_npis[days_ahead:days_ahead + 1], axis=0)
